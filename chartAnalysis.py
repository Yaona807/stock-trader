...
class chartAnalysis:
    ...
    def readImageForUrl(self, chart_url):
        ...
        self.image = self.__resizeImage(
            cv2.imdecode(image_buffer, cv2.IMREAD_UNCHANGED))
    ...
    def analyze(self):
        ...
        analyze_image = cv2.bitwise_and(
            self.image, self.image, mask=image_mask)
        cv2.imshow('', analyze_image)
        cv2.waitKey(0)

        current_line_order = self.__getLineOrder(
            analyze_image, [self.medium_line_color.tolist()])
        best_order = np.array(
            [self.short_line_color, self.bearish_candle_stick_color, self.long_line_color])
    ...
    def __isSlopesUpward(self, image, target_color):
        ...
        for i in range(len(positions) - 1):
            current_slopes = _calcSlopes(
                positions[i][0], positions[i][1], positions[-1][0], positions[-1][1])
